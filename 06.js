...
function get_op (str) {
  return str.match(/(on)|(off)|(toggle)/)[0]
}

function get_xy (str) {
  var from_xy = str.match(/(\d+,\d+)[^\d]+(\d+,\d+)/)[1].split(',')
  from_xy[0] = parseInt(from_xy[0])
  from_xy[1] = parseInt(from_xy[1])

  var to_xy = str.match(/(\d+,\d+)[^\d]+(\d+,\d+)/)[2].split(',')
  to_xy[0] = parseInt(to_xy[0])
  to_xy[1] = parseInt(to_xy[1])
  
  return [from_xy, to_xy]
}

function get_lights (from, to) {
  var lights = []
...
function run_instructions (ops) {
  input.forEach(function (instr) {
    var op = ops[get_op(instr)]

    var start = get_xy(instr)[0]
      , end = get_xy(instr)[1]

    var selected_lights = get_lights(start, end)

    selected_lights.forEach(function (xy) {
      op(xy[0], xy[1])
    })
  })
}
...
init_lights()
run_instructions(ops_v1)

var lights_on = lights.reduce(function (prev, row) {
  return prev + row.filter(function (light) {
    return light == true
  }).length
}, 0)

init_lights()
run_instructions(ops_v2)

var total_brightness = lights.reduce(function (prev, row) {
  return prev + row.reduce(function (prev, light) {
    return light + prev
  }, 0)
}, 0)

console.log(`Total lights on: ${lights_on}`)
