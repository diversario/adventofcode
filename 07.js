...
var ops = {
  ...
  NOT: function(expr) {
    return Math.pow(2,16) + ~value(expr)
  },
  LSHIFT: function (expr, shift) {
    return value(expr) << value(shift)
  },
  RSHIFT: function (expr, shift) {
    return value(expr) >> value(shift)
  }
}
...
function get_rvalue (token_string) {
  return token_string.split(' ').pop()
}
...
function get_tokens (token_string) {
  var token_strings = token_string.split(' ')

  var tokens = []

  while (token_strings.length) {
    let token = token_strings.shift()
...
function tokenizer (token_array) {
  while (token_array.length) {
    let tokens = token_array.shift()
    let rvalue = get_rvalue(tokens)
    ...
    switch (tokens.length) {
      case 3: symbol_table[rvalue] = function(){
        symbol_table[rvalue] = tokens[1](tokens[0], tokens[2])
        return symbol_table[rvalue]
      }
      break
      case 2: symbol_table[rvalue] = function(){
        symbol_table[rvalue] = tokens[0](tokens[1])
        return symbol_table[rvalue]
      }
      break
      case 1: symbol_table[rvalue] = function(){
        symbol_table[rvalue] = tokens[0]()
        return symbol_table[rvalue]
      }
      break
      default: throw new Error('${tokens.length} is invalid length')
    }
  }
}
...
wires.sort().forEach(function(wire){
  let result = value(symbol_table[wire])
